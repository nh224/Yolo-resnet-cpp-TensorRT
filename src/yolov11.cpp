...
YOLOv11::YOLOv11(string model_path, nvinfer1::ILogger& logger) : runtime(nullptr), engine(nullptr), context(nullptr), cpu_output_buffer(nullptr), stream(0)
{
    ...
    else
    {
        ...
        // Save the built engine to a file with the same name as the ONNX file but .engine extension
...
YOLOv11::~YOLOv11()
{
    ...
    // Synchronize and destroy the CUDA stream if it exists
    if (stream != 0)
    {
    ...
    // Free allocated GPU buffers if they exist
    for (int i = 0; i < 2; i++)
    {
    ...
    // Free CPU output buffer if it exists
