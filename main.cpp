...
int main(int argc, char* argv[]) {
    ...
    if (mode == "convert") {
        if (argc != 4) {  // 'convert' requires exactly 4 arguments: program convert onnx_path engine_path
            std::cerr << RED_COLOR << "Usage for conversion: " << RESET_COLOR << argv[0]
                << " convert <onnx_path> <engine_path>" << std::endl;
            return 1;
        }
        onnxPath = inputPath;  // In 'convert' mode, inputPath is the onnx_path
    }
    ...
    if (mode == "convert") {
        try {
            ...
            if (!onnxFile.good()) {
                std::cerr << RED_COLOR << "ONNX file not found: " << onnxPath << RESET_COLOR << std::endl;
                return 1;
            }
            onnxFile.close();
            ...
            // Generate engine path message
            size_t dotIndex = onnxPath.find_last_of(".");
            std::string enginePath = onnxPath.substr(0, dotIndex) + ".engine";
            std::cout << GREEN_COLOR << "Model conversion successful. Engine saved to: " << enginePath << RESET_COLOR << std::endl;
        }
    ...
    // Handle inference modes
        ...
        try {
            ...
            if (mode == "infer_video") {
                ...
                // Preprocess the image with timing
                auto preprocess_start = std::chrono::high_resolution_clock::now();
                yolov11.preprocess(image);
                auto preprocess_end = std::chrono::high_resolution_clock::now();
                double preprocess_time_ms = std::chrono::duration<double, std::milli>(preprocess_end - preprocess_start).count();

                // Perform inference with timing
                auto infer_start = std::chrono::high_resolution_clock::now();
                yolov11.infer();
                auto infer_end = std::chrono::high_resolution_clock::now();
                double infer_time_ms = std::chrono::duration<double, std::milli>(infer_end - infer_start).count();

                // Postprocess to get detections with timing
                std::vector<Detection> detections;
                auto postprocess_start = std::chrono::high_resolution_clock::now();
                yolov11.postprocess(detections);
                auto postprocess_end = std::chrono::high_resolution_clock::now();
                double postprocess_time_ms = std::chrono::duration<double, std::milli>(postprocess_end - postprocess_start).count();

                // Draw detections on the image
                yolov11.draw(image, detections);

                // Display the image (optional) - disabled for headless environment
                ...
                std::cout << GREEN_COLOR << "Preprocess time: " << preprocess_time_ms << " ms" << RESET_COLOR << std::endl;
                std::cout << GREEN_COLOR << "Inference time:  " << infer_time_ms << " ms" << RESET_COLOR << std::endl;
                std::cout << GREEN_COLOR << "Postprocess time: " << postprocess_time_ms << " ms" << RESET_COLOR << std::endl;
                std::cout << GREEN_COLOR << "Total time: " << (preprocess_time_ms + infer_time_ms + postprocess_time_ms) << " ms" << RESET_COLOR << std::endl;
            }
        }
        catch (const std::exception& e) {
